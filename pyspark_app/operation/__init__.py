...
number_npoperator_map = {
    ...
    "not in":lambda val,cond: np.logical_not(np.isin(val,cond)),
...
string_operator_map = {
    ...
    "!=":lambda val,cond:val != cond,
    "<>":lambda val,cond:val != cond,
    "in":lambda val,conds: val in conds if val else False,
    "contain":lambda val,cond:cond in val if val else False,
    "not contain":lambda val,cond:cond not in val if val else True,
    "mcontain":lambda val,cond:any((c in val) for c in cond) if val else False,
    "not mcontain":lambda val,cond:any((c not in val) for c in cond) if val else True,
    "endswith":lambda val,cond: val.endswith(cond) if val else False,
    "not endswith":lambda val,cond: (not val.endswith(cond)) if val else True,
    "mendswith":lambda val,cond:any(val.endswith(c) for c in cond) if val else False,
    "not mendswith":lambda val,cond:(not any(val.endswith(c) for c in cond)) if val else True,
    "startswith":lambda val,cond:val.startswith(cond) if val else False,
    "not startswith":lambda val,cond:(not val.startswith(cond)) if val else True,
    "mstartswith":lambda val,cond:any(val.startswith(c) for c in cond) if val else False,
    "not mstartswith":lambda val,cond:(not any(val.startswith(c) for c in cond)) if val else True,
